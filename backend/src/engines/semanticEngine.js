...
`;
    ...
    /*
     * Automatically ensure standard model for free-tier compatibility
     */
    if (!process.env.GEMINI_MODEL || process.env.GEMINI_MODEL.includes("3.6") || process.env.GEMINI_MODEL.includes("2.5")) {
    ...
    /*
     * Catch rate limit (429) or model missing (404) errors gracefully without failing scan
     */
    if (
      ...
      console.warn(
        "  ⚠️ Semantic Engine: API quota or model unavailable. Skipping semantic scan gracefully."
      );
      return {
        findings: []
      };
